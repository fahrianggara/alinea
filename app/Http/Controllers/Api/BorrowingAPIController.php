...
class BorrowingAPIController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$book) {
            return response()->json(['message' => 'Book not found for borrowing!'], 404);
        }
        ...
        // Inisiasi harga buku
        $bookPrice = 10000; // Contoh harga 10.000 per buku
        ...
        try {
            ...
            Borrowing::create([
                'invoice_id' => $invoice->id, // Menggunakan invoice_id
                'user_id' => Auth::id(),
                'book_id' => $book->id,
                'borrow_date' => $borrowDate,
                'return_date' => $returnDate,
            ...
            // Jika ada kesalahan, rollback transaksi
            DB::rollBack();
            return response()->json(['message' => 'An error occurred while borrowing the book: ' . $e->getMessage()], 500);
        }
    }
